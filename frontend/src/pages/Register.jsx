...
import { Compass, Mail, Lock, User, Eye, EyeOff, ArrowRight, Sparkles, AlertCircle } from "lucide-react";
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || "Registration failed. Please try a different username or email."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-white/15 bg-slate-900/80 p-8 shadow-2xl backdrop-blur-2xl">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="mt-1 text-xs text-slate-400">
            Join RoamRes AI to start engineering personalized travel itineraries.
          </p>
        </div>
